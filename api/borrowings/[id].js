		...
		switch (req.method) {
			case 'GET': {
				...
				if (!ObjectId.isValid(id)) {
					...
					return res
						.status(400)
						.json({ error: 'Nieprawidłowy ID wypożyczenia' })
				}

				const borrowing = await borrowingsCollection.findOne({
					_id: new ObjectId(id),
				})
				...
				console.log(
					'✅ Wypożyczenie pobrane:',
					borrowing.bookId || 'bez bookId',
				)
			...
			case 'DELETE': {
				...
				const result = await borrowingsCollection.deleteOne({
					_id: new ObjectId(id),
				})
				...
				return res
					.status(200)
					.json({ message: 'Wypożyczenie usunięte pomyślnie' })
			}
