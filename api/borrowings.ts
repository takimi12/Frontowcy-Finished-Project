...
export default async function handler(
    ...
    try {
        ...
        const borrowingsCollection: Collection<Borrowing> = db.collection('borrowings')

        switch (req.method) {
            case 'GET': {
                const allBorrowings: Borrowing[] = await borrowingsCollection.find({}).toArray()
            ...
            case 'POST': {
                ...
                if (!newBorrowing.userId || !newBorrowing.bookId || !newBorrowing.borrowDate) {
                    console.log('❌ Brak wymaganych pól dla nowego wypożyczenia.')
                    return res.status(400).json({ error: 'Brak wymaganych pól: userId, bookId i borrowDate.' })
                }

                // Konwersja userId i bookId na ObjectId, jeśli przychodzą jako stringi
                if (typeof newBorrowing.userId === 'string' && ObjectId.isValid(newBorrowing.userId)) {
                    newBorrowing.userId = new ObjectId(newBorrowing.userId)
                }
                if (typeof newBorrowing.bookId === 'string' && ObjectId.isValid(newBorrowing.bookId)) {
                ...
                if (newBorrowing.returnDate && typeof newBorrowing.returnDate === 'string') {
